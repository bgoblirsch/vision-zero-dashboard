...
-- File: schema/fars_crashes_clean.sql
-- Requires: PostGIS extension installed in this database.

CREATE TABLE IF NOT EXISTS fars_crashes_clean (
...
ALTER TABLE fars_crashes_clean
...
CREATE INDEX IF NOT EXISTS crashes_location_gist_idx ON fars_crashes_clean USING GIST (location);
CREATE INDEX IF NOT EXISTS crashes_date_idx ON fars_crashes_clean (crash_date);
CREATE INDEX IF NOT EXISTS crashes_state_idx ON fars_crashes_clean (state_name);
CREATE INDEX IF NOT EXISTS crashes_county_idx ON fars_crashes_clean (county_name);
CREATE INDEX IF NOT EXISTS crashes_city_idx ON fars_crashes_clean (city_name);

COMMENT ON TABLE fars_crashes_clean IS
'FARS crash-level fatalities, normalized across historical schema changes';

COMMENT ON COLUMN fars_crashes_clean.year IS
'Authoritative FARS reporting year; used as primary temporal key';

COMMENT ON COLUMN fars_crashes_clean.location IS
