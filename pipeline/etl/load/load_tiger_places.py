...
def insert_tiger_place(conn: Connection, record: dict) -> bool:
    insert_query = """
        ...
        ON CONFLICT (statefp, placefp) DO NOTHING
        RETURNING 1;
    """
...
def load_tiger_place_features(conn: Connection, gdf: gpd.GeoDataFrame) -> tuple[int, int, int]:
    ...
    for idx, feature in enumerate(gdf.itertuples(), start=1):
        batch_processed += 1
        record = {
            "statefp": feature.STATEFP,
            "placefp": feature.PLACEFP,
        ...
        try:
            ...
            batch_errors += 1
            logger.exception(
                f"[TIGER] Failed to insert place {feature.NAME} ({feature.STATEFP}/{feature.PLACEFP})"
            )
        else:
            if inserted:
                insert_count += 1
...
def ingest_tiger_places(shapefiles: list[Path]) -> tuple[int, int, int]:
    start = time.time()
    logger.info("[TIGER] Starting place ingestion...")

    total_inserted = total_skipped = total_errors = 0

    with get_conn() as conn:
        for shp_path in shapefiles:
            logger.info(f"[TIGER] Loading {shp_path.name}")
            try:
                ...
                logger.info(
                    f"[TIGER] {shp_path.name} — inserted={insert_count}, "
    ...
    logger.info(
        "[TIGER] Ingestion complete. inserted=%s | skipped=%s | errors=%s | duration=%.2fs",
